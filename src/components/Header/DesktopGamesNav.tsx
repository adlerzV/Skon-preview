// components/DesktopGamesNav.tsx
"use client";
...
export default function DesktopGamesNav({ games }: DesktopGamesNavProps) {
  ...
  }, [calculateItems, games.length]);

  const visible = useMemo(() => games.slice(0, visibleCount), [visibleCount, games]);
  const hidden = useMemo(() => games.slice(visibleCount), [visibleCount, games]);
      ...
      <div className="flex items-center h-full w-full">
        ...
        {hidden.length > 0 && (
          <div className="relative group flex items-center h-full w-[50px]">
            ...
            <div className="absolute top-[80px] right-0 bg-[#15171e] border border-white/5 rounded-lg opacity-0 invisible translate-y-1.5 transition-all duration-150 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 p-3 z-[1000] min-w-[280px] shadow-[0_15px_30px_rgba(0,0,0,0.6)] will-change-transform">
              <div className="grid grid-cols-4 gap-2">
                {hidden.map((game) => {
                  const isActive = pathname === game.link || pathname?.startsWith(`${game.link}/`);
                  return (
                    <Link key={game.link} href={game.link} prefetch={false} aria-label={game.title} className={`flex items-center justify-center p-2 rounded transition-colors group/game ${isActive ? "bg-white/10" : "hover:bg-white/5"}`}>
